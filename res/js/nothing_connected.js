...
async function scanNewDevices() {
  ...
  if (sppPort) {
    console.log(
      "connected to a Bluetooth Serial Port Profile port",
      sppPort.getInfo()
    );
...
function getSerialNumber(hexPayload) {
  ...
  const serialConfigs = configurations.filter(
    (config) => config.type === 4 && config.value.length > 0
  );
...
async function scanNewDevicesSerial() {
  ...
  try {
    ...
    document.getElementById("device_container").innerHTML =
      '<div class="device-info"><p>Device not selected</p></div>';
    setTimeout(function () {
      window.location.reload();
    }, 3000);
    return;
