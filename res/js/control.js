...
function updateBattery(leftText, caseText, rightText) {
  ...
  containerRight.style.display =
    rightText === "DISCONNECTED" ? "none" : "block";
...
function getDevicesForList(devices) {
  ...
  for (var i = 0; i < devices.length; i++) {
    ...
    container.className =
      "inline-grid p-2 pl-5 pr-5 cursor-pointer border-[1px] border-black rounded-xl mt-2 hover:scale-[105%] duration-200 ease-in-out";
...
function setFirmwareText(firmware_text) {
  document.getElementById("settings_subtitle_firmware").innerHTML =
    firmware_text;
